...
if submit_btn:
    ...
    # 🌟 1. Feature Engineering (คำนวณคณิตศาสตร์จากข้อความตรงๆ ไม่ต้องแปลง 1/0)
    # อัปเดตชื่อคอลัมน์เป็น รายได้คงเหลือหลังหักหนี้
    df['รายได้คงเหลือหลังหักหนี้'] = df['รายได้'] - df['ชำระต่องวด']
    df['Cov_Ratio'] = np.where(df['ชำระต่องวด'] > 0, df['รายได้คงเหลือหลังหักหนี้'] / df['ชำระต่องวด'], 0)
    df['DTI'] = np.where(df['รายได้'] > 0, (df['ชำระต่องวด'] / df['รายได้']) * 100, 0)

    # เช็กเงื่อนไขวงเงินค้ำประกัน (ใช้ตัวหนังสือ 'มี', 'ไม่มี' ได้เลย)
    ...
    with col_f1:
        # อัปเดตให้ดึงข้อมูลจากตัวแปรชื่อใหม่
        st.metric(label="รายได้คงเหลือหลังหักหนี้", value=f"{df['รายได้คงเหลือหลังหักหนี้'][0]:,.2f} บาท")
        dti_val = df['DTI'][0]
    ...
    # 🌟 2. จัดโครงสร้างข้อมูลให้ตรงกับที่ LightGBM จำได้ 100%
    # ทริค: ดึงรายชื่อคอลัมน์จากตัวโมเดลเลย ป้องกันการสลับตำแหน่ง
    expected_cols = model.feature_name_
    X_ready = df.reindex(columns=expected_cols, fill_value=0)

    # 🌟 3. แปลงคอลัมน์ที่เป็นตัวหนังสือให้กลายเป็น dtype 'category'
    cat_columns = [
        'เพศ', 'อาชีพ', 'ประเภทรายได้', 'หลักค้ำบุคคล', 'อาชีพผู้ค้ำ',
        'หลักค้ำโฉนด', 'หลักค้ำเงินฝาก', 'ชื่อกลุ่ม', 'อาชีพหัวหน้ากลุ่ม',
